...
const getKeys = async () => {
  console.log("publish key ====>", process.env.REACT_APP_PUB_KEY);
  console.log("subscribe key ====>", process.env.REACT_APP_SUB_KEY);
  return {
...
const main = async () => {
  ...
  while (
    !(
      keys.publishKey.startsWith("pub-") && keys.subscribeKey.startsWith("sub-")
    )
  ) {
    ...
    if (!answer) {
      console.log(
        `Please add your publish and subscribe key and re-run the script.`
      );
      process.exit(0);
    }
    keys = await getKeys();
  ...
  const userMemberships = users
    .map(({ id: uuid }) =>
      batch(sampleSize(channels, membershipsPerUser), batchSize).map(
        (channel) => ({
          uuid,
          channels: channel,
        })
  ...
  if (timeouts > 0) {
    console.log(
      ...
      node populate-pn-objects.js --batch-size ${Math.ceil(
        batchSize / 2
      )} --request-interval ${requestInterval + 100}`
    );
  }
